// M3-CURVE -- M3 + shared population curvature gamma*log(age/a0)^2 (debug: does
// adding curvature absorb log-linear misfit and reduce sigma_b?). Accelerating accumulator + between-child EFFICIENCY and ACCELERATION
...
parameters {
  ...
  real gamma;                                  // population curvature on L^2
}
...
transformed parameters {
  ...
  for (a in 1:A) {
    ...
    admin_base[a] = xi[ch] + log_H + kappa[ch] * (log(fmax(admin_age[a],0.01)/a0)) + gamma * square(log(fmax(admin_age[a],0.01)/a0));
  }
