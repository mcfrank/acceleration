...
parameters {
  ...
  vector[D] mu_xi;
  vector[D] delta;
  // Partial-pooled child scales, CENTERED in log space.
  // m_pool.stan declared these non-centred (sigma = exp(m + s*z), z ~ std_normal).
  // That is the right choice when each group is weakly informed, but here every dataset
  // contributes 96-792 children, so the likelihood pins each sigma[d] tightly and the
  // non-centred form induces a funnel between s and z: rhat reached 2.15 with ess ~5 and
  // 3997 of 4000 transitions saturating treedepth. With strongly-informed groups the
  // centred parameterisation is the better-conditioned one.
  real m_a; real<lower=0> s_a; vector[D] log_sigma_a;
  real m_b; real<lower=0> s_b; vector[D] log_sigma_b;
...
transformed parameters {
  vector<lower=0>[D] sigma_a = exp(log_sigma_a);   // pooled scales (centred)
  vector<lower=0>[D] sigma_b = exp(log_sigma_b);
...
model {
  ...
  // log-scale hierarchy, centred: the prior IS on the declared parameter, so no Jacobian
  log_sigma_a ~ normal(m_a, s_a);
  log_sigma_b ~ normal(m_b, s_b);
